...
|-------|----------|-----------------|--------|
| Checks | math | <span title=":: evenQ(x) ::">[`evenQ`](#evenQ)</span> <span title=":: oddQ(x) ::">[`oddQ`](#oddQ)</span> <span title=":: zeroQ(x) :: checks directly via (= x 0)">[`zeroQ`](#zeroQ)</span> <span title=":: negativeQ(x) :: checks directly via (< x 0)">[`negativeQ`](#negativeQ)</span> <span title=":: positiveQ(x) :: checks directly via (> x 0)">[`positiveQ`](#positiveQ)</span> |  |
| Checks | type checks | <span title="">[`noneQ`](#noneQ)</span> <span title="">[`notnoneQ`](#notnoneQ)</span> <span title=":: oftypeQ(tp, x) :: checks directly via (= (type x) tp)">[`oftypeQ`](#oftypeQ)</span> <span title=":: ofinstQ(tp, x) :: checks directly via (isinstance x tp)">[`ofinstQ`](#ofinstQ)</span> <span title=":: intQ(x) :: checks literally if type(x) == int, will also work with StrictInt from pydantic">[`intQ`](#intQ)</span> <span title=":: floatQ(x) :: checks literally if type(x) == float, will also work with StrictFloat from pydantic">[`floatQ`](#floatQ)</span> <span title=":: numberQ(x) :: checks for intQ or floatQ, will also work with StrictInt/StrictFloat from pydantic">[`numberQ`](#numberQ)</span> <span title=":: strQ(x) :: checks literally if type(x) == str, will also work with StrictStr from pydantic">[`strQ`](#strQ)</span> <span title=":: dictQ(x) :: checks literally if type(x) == dict">[`dictQ`](#dictQ)</span> <span title=":: listQ(value) :: checks if value is list">[`listQ`](#listQ)</span> <span title=":: tupleQ(value) :: checks if value is tuple">[`tupleQ`](#tupleQ)</span> <span title=":: setQ(value) :: checks if value is set">[`setQ`](#setQ)</span> <span title=":: iteratorQ(value) :: checks if value is iterator">[`iteratorQ`](#iteratorQ)</span> <span title=":: iterableQ(value) :: checks if value is iterable">[`iterableQ`](#iterableQ)</span> |  |
| Checks | etc | <span title=":: trueQ(x) :: checks directly via (= x True)">[`trueQ`](#trueQ)</span> <span title=":: falseQ(x) :: checks directly via (= x False)">[`falseQ`](#falseQ)</span> <span title=":: oflenQ(n, xs) :: checks directly via (= (len xs) n)">[`oflenQ`](#oflenQ)</span> <span title=":: zerolenQ(xs) :: checks directly via (= (len xs) 0)">[`zerolenQ`](#zerolenQ)</span> |  |
| Math | basics | <span title=":: inc(n) :: = n + 1">[`dec`](#dec)</span> <span title=":: dec(n) :: = n - 1">[`inc`](#inc)</span> <span title=":: sign(n) :: will give 0 for n=0">[`sign`](#sign)</span> |  |
| Math | multiples | <span title=":: half(x) :: = x/2">[`half`](#half)</span> <span title=":: double(x) :: = x*2">[`double`](#double)</span> <span title=":: reciprocal(x) :: = 1/x ; throws error for x=0">[`reciprocal`](#reciprocal)</span> |  |
| Math | rounding | <span title="floor(1.9) = 1">[`floor`](#floor)</span> <span title="ceil(1.1) = 2">[`ceil`](#ceil)</span> <span title=":: round_to(n, step) :: rounds to closest multiple of step: round_to(9.1, 1.5) == 9.0">[`round_to`](#round_to)</span> <span title=":: clip(x, lower, upper) :: clips x to fit in lower <= x <= upper limit">[`clip`](#clip)</span> <span title="approx_eq(a, b, *, rel_tol=1e-09, abs_tol=0.0)">[`approx_eq`](#approx_eq)</span> |  |
| Math | squaring | <span title=":: sqrt(n) :: = √n">[`sqrt`](#sqrt)</span> <span title=":: squared(x) :: = pow(x,2)">[`squared`](#squared)</span> <span title=":: dist(p, q) -> float :: ≈ √((px-qx)² + (py-qy)² ...)">[`dist`](#dist)</span> <span title=":: hypot(*coordinates) :: = √(x² + y² + ...)">[`hypot`](#hypot)</span> <span title=":: normalize(xs) -> xs :: will throw error for zero-len vector">[`normalize`](#normalize)</span> |  |
| Math | exponents | <span title=":: exp(x) ::">[`exp`](#exp)</span> <span title=":: log(x, base=math.e) ::">[`log`](#log)</span> <span title=":: ln(x) :: = math.log(x, math.e) ; coexists with log for clarity">[`ln`](#ln)</span> <span title=":: log10(x) ::">[`log10`](#log10)</span> |  |
| Math | trigonometry | <span title="literally just float pi=3.14...">[`pi`](#pi)</span> <span title=":: sin(x) :: x is in radians">[`sin`](#sin)</span> <span title=":: cos(x) :: x is in radians">[`cos`](#cos)</span> <span title=":: tan(x) :: x is in radians, will give smth like 1.6E+16 for x = pi">[`tan`](#tan)</span> <span title=":: degrees(x) :: x in radians is converted to degrees">[`degrees`](#degrees)</span> <span title=":: radians(x) :: x in degrees is converted to radians">[`radians`](#radians)</span> <span title=":: acos(x) :: x is in radians, result is between 0 and pi">[`acos`](#acos)</span> <span title=":: asin(x) :: x is in radians, result is between -pi/2 and pi/2">[`asin`](#asin)</span> <span title=":: asin(x) :: x is in radians, result is between -pi/2 and pi/2">[`atan`](#atan)</span> <span title=":: atan2(y, x) :: both signs are considered">[`atan2`](#atan2)</span> |  |
| Math | random | <span title=":: choice(seq) -> Elem :: throws error for empty list">[`choice`](#choice)</span> <span title=":: randint(a, b) -> int :: returns random integer in range [a, b] including both end points">[`randint`](#randint)</span> <span title=":: randfloat(a, b) -> float :: range is [a, b) or [a, b] depending on rounding">[`randfloat`](#randfloat)</span> <span title=":: rand01() -> float :: generates random number in interval [0, 1)">[`rand01`](#rand01)</span> |  |
| Math | reductions | <span title=":: sums(seq [, acc]) -> generator :: reductions with addition function">[`sums`](#sums)</span> <span title=":: lsums(seq [, acc]) -> list :: list version of sums">[`lsums`](#lsums)</span> <span title=":: product(iterable, /, *, start=1) :: product([2, 3, 5]) = 30">[`product`](#product)</span> |  |
| Operator to functions | math | <span title=":: minus(x, y) = x - y ::">[`minus`](#minus)</span> <span title=":: neg(n) :: = -1 * n">[`neg`](#neg)</span> <span title=":: mod(5, 2) :: = 1">[`mod`](#mod)</span> <span title="'@' as function">[`matmul`](#matmul)</span> <span title=":: div(a, b) ::">[`div`](#div)</span> |  |
| Operator to functions | logics | <span title="'and' as function">[`and_`](#and_)</span> <span title="'or' as function">[`or_`](#or_)</span> <span title="'not' as function">[`not_`](#not_)</span> <span title="'is' as function">[`is_`](#is_)</span> <span title="">[`xor`](#xor)</span> <span title="equal">[`eq`](#eq)</span> <span title="non-equal">[`neq`](#neq)</span> <span title="greater than">[`gt`](#gt)</span> <span title="less than">[`lt`](#lt)</span> <span title="greater or equal">[`geq`](#geq)</span> <span title="less or equal">[`leq`](#leq)</span> <span title=":: gt0(x) :: checks for x > 0">[`gt0`](#gt0)</span> <span title=":: geq0(x) :: x >= 0">[`geq0`](#geq0)</span> <span title=":: lt0(x) :: checks for x < 0">[`lt0`](#lt0)</span> <span title=":: leq0(x) :: x <= 0">[`leq0`](#leq0)</span> |  |
| Operator to functions | dunders | <span title=":: mul(*args) :: literally just mul(a,b,c,...)=a*b*c*...; can also be used with 0 or 1 arg">[`mul`](#mul)</span> <span title=":: smul(*args) :: synonim of mul (with underlined usage on strings)">[`smul`](#smul)</span> <span title=":: lmul(*args) :: synonim of mul (with underlined usage on lists)">[`lmul`](#lmul)</span> <span title=":: plus(*args) :: literally just plus(a,b,c,...)=a+b+c...; can also be used with 0 or 1 arg">[`plus`](#plus)</span> <span title=":: sconcat(*args) :: string concantenation as a monoid (will not give error when used with 0 or 1 args)">[`sconcat`](#sconcat)</span> |  |
...
| Strings | methods to functions | <span title=":: lowercase(string) :: str.lower method as a function">[`lowercase`](#lowercase)</span> <span title=":: strip(string, chars=None) :: str.strip method as a function">[`strip`](#strip)</span> <span title=":: lstrip(string, chars=None) :: str.lstrip method as a function">[`lstrip`](#lstrip)</span> <span title=":: rstrip(string, chars=None) :: str.rstrip method as a function">[`rstrip`](#rstrip)</span> |  |
...
| Misc | benchmarking | <span title=":: cur_time() :: gets current time in seconds">[`cur_time`](#cur_time)</span> <span title=":: dt_printer(* args, fresh_run=False) :: starts timer on fresh run, prints time passed since previous call">[`dt_print`](#dt_print)</span> | <span title=":: (timing expr1 expr2 ...) -> #(float, Any) :: returns time (in seconds) and result of execution of (fn [] expr1 expr2 ...)">[`timing`](#timing)</span> |
| Misc | misc | <span title="standard python pprint function">[`pprint`](#pprint)</span> <span title=":: lprint(seq, sep=None) :: prints every elem of seq on new line">[`lprint`](#lprint)</span> | <span title="">[`comment`](#comment)</span> |
| Misc | testing |  | <span title=":: (assertm op arg1 arg2) :: tests if (op arg1 arg2), for example (= 1 1)">[`assertm`](#assertm)</span> <span title="example: (assertm gives_error_typeQ (get [1] 2) IndexError)">[`gives_error_typeQ`](#gives_error_typeQ)</span> |
...
```hy
Help on function zeroQ in module fpmx.prelude.funcs:
...
```hy
Help on function negativeQ in module fpmx.prelude.funcs:
...
```hy
Help on function positiveQ in module fpmx.prelude.funcs:
...
```hy
Help on function oftypeQ in module fpmx.prelude.funcs:
...
```hy
Help on function ofinstQ in module fpmx.prelude.funcs:
...
```hy
Help on function intQ in module fpmx.prelude.funcs:
...
```hy
Help on function floatQ in module fpmx.prelude.funcs:
...
```hy
Help on function numberQ in module fpmx.prelude.funcs:
...
```hy
Help on function strQ in module fpmx.prelude.funcs:
...
```hy
Help on function dictQ in module fpmx.prelude.funcs:
...
```hy
Help on function trueQ in module fpmx.prelude.funcs:
...
```hy
Help on function falseQ in module fpmx.prelude.funcs:
...
```hy
Help on function oflenQ in module fpmx.prelude.funcs:
...
```hy
Help on function zerolenQ in module fpmx.prelude.funcs:
...
```hy
Help on function dec in module fpmx.prelude.funcs:
...
```hy
Help on function inc in module fpmx.prelude.funcs:
...
```hy
Help on function sign in module fpmx.prelude.funcs:
...
```hy
Help on function half in module fpmx.prelude.funcs:
...
```hy
Help on function double in module fpmx.prelude.funcs:
...
```hy
Help on function reciprocal in module fpmx.prelude.funcs:
...
```hy
Help on function round_to in module fpmx.prelude.funcs:
...
```hy
Help on function clip in module fpmx.prelude.funcs:
...
```hy
Help on function squared in module fpmx.prelude.funcs:
...
```hy
Help on function normalize in module fpmx.prelude.funcs:
...
```hy
Help on function ln in module fpmx.prelude.funcs:
...
```hy
Help on function minus in module fpmx.prelude.funcs:
...
```hy
Help on function gt0 in module fpmx.prelude.funcs:
...
```hy
Help on function geq0 in module fpmx.prelude.funcs:
...
```hy
Help on function lt0 in module fpmx.prelude.funcs:
...
```hy
Help on function leq0 in module fpmx.prelude.funcs:
...
```hy
Help on function mul in module fpmx.prelude.funcs:
...
```hy
Help on function smul in module fpmx.prelude.funcs:
...
```hy
Help on function lmul in module fpmx.prelude.funcs:
...
```hy
Help on function plus in module fpmx.prelude.funcs:
...
```hy
Help on function sconcat in module fpmx.prelude.funcs:
...
```hy
Help on function strlen in module fpmx.prelude.funcs:
...
```hy
Help on function str_join in module fpmx.prelude.funcs:
...
```hy
Help on function lowercase in module fpmx.prelude.funcs:
...
```hy
Help on function strip in module fpmx.prelude.funcs:
...
```hy
Help on function lstrip in module fpmx.prelude.funcs:
...
```hy
Help on function rstrip in module fpmx.prelude.funcs:
...
```hy
Help on function third in module fpmx.prelude.funcs:
...
```hy
Help on function fourth in module fpmx.prelude.funcs:
...
```hy
Help on function beforelast in module fpmx.prelude.funcs:
...
```hy
Help on function rest in module fpmx.prelude.funcs:
...
```hy
Help on function butlast in module fpmx.prelude.funcs:
...
```hy
Help on function drop in module fpmx.prelude.funcs:
...
```hy
Help on function take in module fpmx.prelude.funcs:
...
```hy
Help on function pick in module fpmx.prelude.funcs:
...
Info: accepts fpmx-style .arg syntax
```
...
Info: accepts fpmx-style .attr syntax
```
...
```hy
Help on function lmask_sel in module fpmx.prelude.funcs:
...
```hy
Help on function mask2idxs in module fpmx.prelude.funcs:
...
```hy
Help on function idxs2mask in module fpmx.prelude.funcs:
...
```hy
Help on function lcycle in module fpmx.prelude.funcs:
...
```hy
Help on function lrepeat in module fpmx.prelude.funcs:
...
```hy
Help on function lislice in module fpmx.prelude.funcs:
...
```hy
Help on function lconcat in module fpmx.prelude.funcs:
...
```hy
Help on function lreversed in module fpmx.prelude.funcs:
...
```hy
Help on function fltr1st in module fpmx.prelude.funcs:
...
```hy
Help on function without in module fpmx.prelude.funcs:
...
```hy
Help on function lwithout in module fpmx.prelude.funcs:
...
```hy
Help on function flatten in module fpmx.prelude.from_hyrule:
...
```hy
Help on function lbisect_at in module fpmx.prelude.funcs:
...
```hy
Help on function partition in module fpmx.prelude.funcs:
...
```hy
Help on function lpartition in module fpmx.prelude.funcs:
...
```hy
Help on function lmulticut_by in module fpmx.prelude.funcs:
...
```hy
Help on function lzip in module fpmx.prelude.funcs:
...
```hy
Help on function lstarmap in module fpmx.prelude.funcs:
...
```hy
Help on function constantly in module fpmx.prelude.funcs:
...
```hy
Help on function nested in module fpmx.prelude.funcs:
...
```hy
Help on function apply_n in module fpmx.prelude.funcs:
...
```hy
Help on function flip in module fpmx.prelude.funcs:
...
```hy
Help on function pflip in module fpmx.prelude.funcs:
...
```hy
Help on function fnot in module fpmx.prelude.funcs:
...
```hy
Help on function eq_any in module fpmx.prelude.funcs:
...
```hy
Help on function on in module fpmx.prelude.funcs:
...
```hy
Help on function all_fs in module fpmx.prelude.funcs:
...
```hy
Help on function any_fs in module fpmx.prelude.funcs:
...
```hy
Help on function get_ in module fpmx.prelude.funcs:
...
```hy
Help on function nth_ in module fpmx.prelude.funcs:
...
```hy
Help on function slice_ in module fpmx.prelude.funcs:
...
```hy
Help on function cut_ in module fpmx.prelude.funcs:
...
```hy
Help on function dt_print in module fpmx.prelude.funcs:
...
```hy
Help on function lprint in module fpmx.prelude.funcs:
