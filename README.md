...
- **fpmx.prelude (the Prelude)** — the stable heart of the library.
...
- **fpmx.extras** — a collection of modules that contain specialized,
  ...
  without bloating the prelude prelude or compromising its stability.
...
[Prelude cheatcheet](docs/prelude_cheatsheet_autogenerated.md) 
...
Since fpmx is language extension, rather than just a collection of functions and macros,
it offers alternative concise syntax for loading it's modules.

To load only prelude:
```hy
(require fpmx.loader [load_fpmx])
(load_fpmx "prelude")
...
1. Prelude:
   ...
   - [Cheatsheet (one-liners form)](docs/01_prelude_cheatsheet_list_view.hy) 
   - [Prelude macros in details](docs/02_prelude_macros.md) 
2. Extra modules:
